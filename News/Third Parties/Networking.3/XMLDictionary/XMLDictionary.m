...
//
//  Modified by Ahmad al-Moraly on 10/4/2012.
//  Copyright 2012 Artgin. All rights reserved.
//
...
+ (NSMutableDictionary *)dictionaryWithXMLData:(NSData *)data andError:(NSError **)error;
...
{	
	...
	return [self dictionaryWithXMLData:data andError:nil];
}
...
{	
	...
	if (!self.top)
	{
		self.root = [NSMutableDictionary dictionaryWithObject:node forKey:elementName];// node;
...
{	
		...
		self.top.innerText)
	{
        ...
        if (node.count == 1) {
            ...
            if (nodeName)
            {
                [self.top setObject:@"" forKey:nodeName];
            }
        }
	}
}
...
+ (NSDictionary *)dictionaryWithXMLData:(NSData *)data andError:(NSError **)error
{
	return [XMLDictionaryParser dictionaryWithXMLData:data andError:error];
}

+ (NSDictionary *)dictionaryWithXMLString:(NSString *)string
{
	NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
	return [XMLDictionaryParser dictionaryWithXMLData:data andError:nil];
}
